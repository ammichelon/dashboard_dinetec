...
// ✅ ADMIN reset password (default: 2609)
const RESET_PASSWORD = String(process.env.RESET_PASSWORD || "2609");
...
(async () => {
  ...
  async function resetDatabase() {
    // transação pra garantir atomicidade
    await db.exec("BEGIN IMMEDIATE;");
    try {
      await db.exec("DELETE FROM checkins;");
      await db.exec("DELETE FROM leads;");
      // reseta autoincrement
      try {
        await db.exec("DELETE FROM sqlite_sequence WHERE name IN ('leads','checkins');");
      } catch {}
    ...
    // VACUUM fora da transação (SQLite exige)
    try { await db.exec("VACUUM;"); } catch {}
  }

  // ✅ Página simples com popup (confirm + senha)
...
<head>
  <meta charset="utf-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1"/>
  <title>Reset DB • Boa Safra</title>
  <style>
    body{font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial;margin:0;padding:18px;background:#f6faf8;color:#0f172a}
    .wrap{max-width:520px;margin:0 auto}
    .card{background:#fff;border:1px solid #e7edf0;border-radius:16px;padding:16px;box-shadow:0 10px 22px rgba(0,0,0,.08)}
    h1{margin:0 0 8px;color:#0b5a2a;font-size:18px}
    p{margin:0 0 14px;color:#5b677a}
    button{width:100%;padding:12px;border:0;border-radius:12px;font-weight:900;font-size:15px;color:#fff;background:linear-gradient(90deg,#0b5a2a,#1f7a36);cursor:pointer}
    .muted{margin-top:10px;font-size:12px;color:#7a8799}
    .ok{color:#0b5a2a;font-weight:900}
...
<body>
  <div class="wrap">
    <div class="card">
      <h1>Resetar banco de dados</h1>
      <p>Isso vai apagar <b>todos</b> os leads e check-ins do Railway.</p>
      <button id="btn">Resetar agora</button>
  ...
  <script>
    ...
    btn.addEventListener('click', async () => {
      msg.textContent = '';
      const ok = confirm('Tem certeza? Isso apaga TUDO do banco do Railway.');
      if(!ok) return;

      const pw = prompt('Digite a senha para confirmar:');
      if(pw === null) return;
      ...
      try{
        ...
        if(j.ok){
          msg.innerHTML = '<span class="ok">OK ✅ Banco resetado.</span> Recarrega o dashboard.';
        }else{
          msg.innerHTML = '<span class="err">Falhou:</span> ' + (j.error || 'erro');
        }
      }catch(e){
        msg.innerHTML = '<span class="err">Erro de conexão.</span>';
      }finally{
        btn.disabled = false;
        btn.style.opacity = 1;
      }
  ...
  // ✅ Endpoint do reset (com senha)
  app.post("/api/admin/reset", async (req, res) => {
    try {
      const pw = String(req.body?.password || "");
      if (pw !== RESET_PASSWORD) return res.status(401).json({ ok: false, error: "bad_password" });
  ...
  app.post("/api/ingest", async (req, res) => {
    try {
      ...
      return res.json({
        ...
        leads_saved: leadsOk,
        checkins_received: checkins.length,
        checkins_saved: checkinsOk,
      });
    } catch {
  ...
  async function createLead(payload, origem) {
    ...
    const sementes_atuais = Array.isArray(payload.sementes_atuais)
      ? JSON.stringify(payload.sementes_atuais)
      : (payload.sementes_atuais || null);

    const sementes_outras = (payload.sementes_outras || "").trim() || null;

    const culturas = Array.isArray(payload.culturas)
      ? JSON.stringify(payload.culturas)
      : (payload.culturas || null);
  ...
  async function maybeCreateCheckin(leadId) {
    ...
    const last = await db.get(
      "SELECT created_at FROM checkins WHERE lead_id = ? ORDER BY created_at DESC LIMIT 1",
      leadId
    );
    if (last?.created_at) {
    ...
    await db.run(
      "INSERT INTO checkins (lead_id, origem, created_at, day_key, hour) VALUES (?, 'wifi', ?, ?, ?)",
